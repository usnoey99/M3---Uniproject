 ...
 * 
 * Änderungen:
 * 1. Nach Schwierigkeitsauswahl im Menü wird das Display schwarz bevor das Spiel startet.
 * 2. NeoPixel LEDs kommunizieren die Beat-Richtung an den Spieler:
 *   - LED 0 = Rechts (directionOffset 0)
 *   - LED 1 = Oben (directionOffset 1)
 *   - LED 2 = Links (directionOffset 2)
 *   - LED 3 = Unten (directionOffset 3)
 *   - LED 4 = Feedback (Grün = gut, Gelb = ok, Rot = schlecht/verpasst)
 * 
 * Farbe der aktiven Richtungs-LED:
 *   - Easy Mode: Grün
 *   - Hard Mode: Rot
 *
 *   Alle anderen Richtungs-LEDs bleiben aus.
 *
 */
 
// We need the Adafruit Arcada Library for our basic functionality
#include <Adafruit_Arcada.h>
#include <Adafruit_NeoPixel.h> // new library
...
// Definition Neopixel
// PyBadge hat 5 NeoPixel LEDs, angeschlossen an Pin 8
#define PIN 8 // Pin für Neopixel-Daten
#define NUMPIXELS 5 // Anzahl der Neopixel-LEDs
Adafruit_NeoPixel pixels(NUMPIXELS, PIN, NEO_GRB + NEO_KHZ800);

// NeoPixel LED-Index Zuweisung für Richtungen
#define LED_RIGHT 0 // directionOffset 0 -> Rechts -> LED 0
#define LED_UP 1 // directionOffset 1 -> Oben -> LED 1
#define LED_LEFT 2 // directionOffset 2 -> Links -> LED 2
#define LED_DOWN 3 // directionOffset 3 -> Unten -> LED 3
#define LED_FEEDBACK 4 // LED 4 -> Feedback (Timing-Qualität)

// Farben für Feedback-LED
#define COLOR_GOOD pixels.Color(0, 255, 0) // Grün = perfektes Timing
#define COLOR_OK pixels.Color(255, 200, 0) // Gelb = akzeptables Timing
#define COLOR_BAD pixels.Color(255, 0, 0) // Rot = schlechtes Timing oder verpasst
#define COLOR_OFF pixels.Color(0, 0, 0) // Aus

// Helligkeit der Richtungs-LED (0–255)
#define BRIGHTNESS_DIRECTION 180
...
void setup() {
  ...
  pixels.begin();
  pixels.setBrightness(BRIGHTNESS_DIRECTION);
  pixels.clear();
  pixels.show(); // Alle LEDs auf AUS setzen
...
/**
 * Schwarzer Übergangsbildschirm zwischen Menü und Spiel.
 * Display wird schwarz, dann startet das Spiel direkt.
 */
void handleBlack() {
  // Display schwarz schalten
...
/**
 * Setzt eine einzelne NeoPixel-LED für die aktuelle Beat-Richtung.
 * 
 * Mapping:
 *  directionOffset 0 -> LED_RIGHT (LED 0)
 *  directionOffset 1 -> LED_UP (LED 1)
 *  directionOffset 2 -> LED_LEFT (LED 2)
 *  directionOffset 3 -> LED_DOWN (LED 3)
 * 
 * Farbe:
 *  Easy Mode -> Blau
 *  Hard Mode -> Lila
...
void updateDirectionLED(uint8_t direction) {
  // Alle Richtungs-LEDs (0–3) ausschalten
  for (int i = 0; i < 4; i++) {
    pixels.setPixelColor(i, COLOR_OFF);
  }

  // Aktive Richtungs-LED einschalten
  uint32_t dirColor;
  if (easyMode) {
    dirColor = pixels.Color(0, 100, 255); // Blau für Easy
  } else {
    dirColor = pixels.Color(180, 0, 255); // Lila für Hard
  }

  // direction 0-3 entspricht direkt LED-Index 0-3
  if (direction < 4) {
    pixels.setPixelColor(direction, dirColor);
  }
...
/**
 * Setzt die Feedback-LED (LED 4) basierend auf dem Timing.
 * 
 * Wird nach jedem Knopfdruck aufgerufen:
 *  |timingOffset| < 50 -> Grün (gut)
 *  |timingOffset| < 90 -> Gelb (ok)
 *  sonst -> Rot (schlecht)
 * 
 * Bei verpasstem Beat -> Rot.
 */
void updateFeedbackLED(bool pressed, int32_t timing) {
  if (!pressed) {
    // Beat verpasst
    pixels.setPixelColor(LED_FEEDBACK, COLOR_BAD);
  } else {
    if (abs(timing) < 50) {
      pixels.setPixelColor(LED_FEEDBACK, COLOR_GOOD);
    } else if (abs(timing) < 90) {
      pixels.setPixelColor(LED_FEEDBACK, COLOR_OK);
    } else {
      pixels.setPixelColor(LED_FEEDBACK, COLOR_BAD);
    }
  }
...
* 
 * NeoPixel Änderungen:
 *  directionOffset -> wird auf LED 0-3 gemappt (Richtungsanzeige)
 *  Timing-Feedback -> LED 4 (Feedback-LED)
 */
void handleGameplay() {
  // music main loop
  if (musicIsPlaying) {
  ...
  if (timeNow - beatTime > 0 && timeNow - beatTime < 100) {
    updateFeedbackLED(buttonPressedForLastBeat, timingOffset); 
  } else if (timeNow - beatTime >= 100) {
    pixels.setPixelColor(LED_FEEDBACK, COLOR_OFF);
    pixels.show();
  }
  // NeoPixel: Richtungs-LED aktualisieren
  updateDirectionLED(directionOffset);

  // Display: schwarzer Hintergrund
  canvas->fillScreen(ARCADA_BLACK);
