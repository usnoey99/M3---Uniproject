...
int32_t get_audio_data(Frame *channels, int32_t channel_len) {
    ...
    for (int i = 0; i < channel_len; i++) {
        ...
        // Snare-Ton (kurz, mit Decay-Effekt,
        if (snare_current_frequency > 0 && (millis() - snare_start_time < snare_duration)) {
            float progress = (float)(millis() - snare_start_time) / (float)snare_duration;
            float decay    = 1.0f - progress;
            sample += (int16_t)(sin(snare_index) * 1200 * decay);
            snare_index += (2 * M_PI * snare_current_frequency) / 44100.0;
            if (snare_index >= 2 * M_PI) snare_index -= 2 * M_PI;
        } else {
            snare_index = 0;
...
//zuerts subbeat?
        ...
        current_subbeat++;
                ...
                if (snare_bits_active & (1 << current_subbeat)) {
                    ...
                    snare_duration          = subbeat_interval * 0.2;
