...
void loop() {
    ...
    if (String(state.status) == "RUNNING" && track_length > 0) {
        ...
        if (current_time - last_beat_time >= beat_interval) {
            ...
            last_subbeat_ms = current_time;
            current_subbeat = 0;  // Erstes Viertel

            // Kick live lesen mit Watchdog gerade kein watchsog für test
            // Wenn Modul nicht antwortet → Fehlerzaehler erhoehen
        readKickLive(playhead);
         

            // Kick-Bitmuster fuer diesen Beat merken
            // Wird fuer Sub-Beats benoetigt
            kick_bits_active = kick_pattern[playhead];
            ...
            if (kick_bits_active & 0x01) {
                ...
                Serial.printf("[KICK] Viertel 1\n");
            }

            Serial.printf("[BEAT] Pos:%d/%d | %s | Note:%s | kick=0b%d%d%d%d\n",
                playhead + 1, track_length,
            ...
            // Playhead weiterruecken
            playhead++;
            if (playhead >= track_length) {
                playhead           = 0;              // Von vorne anfangen
                current_note_index = start_note_index;  // Note zurueck auf Startnote
            }

            updateNeoPixelTrack();  // Auto-Position auf LEDs aktualisieren
        }

        // Sub-Beats 2, 3, 4 (Viertel 2-4 innerhalb des Beats)
        // Sub-Beat 1 (Viertel 1) wird oben beim Haupt-Beat ausgeloest
       
        if (subbeat_interval > 0 && current_subbeat < 3) {
            if (current_time - last_subbeat_ms >= subbeat_interval) {
                last_subbeat_ms = current_time;
                current_subbeat++;  // Naechstes Viertel (1, 2, 3)

            
                // Bit 1 = Viertel 2, Bit 2 = Viertel 3, Bit 3 = Viertel 4
                if (kick_bits_active & (1 << current_subbeat)) {
                    kick_current_frequency = KICK_FREQUENCY;
                    kick_start_time        = current_time;
                    kick_duration          = subbeat_interval * 0.2;
                    Serial.printf("[KICK] Viertel %d\n", current_subbeat + 1);
                }
            }
        }
    }
